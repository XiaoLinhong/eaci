module mod_csvio
    ...
    type(pointInfo) function read_info(vFile, flag) result(info)
        ...
        call info%locs%reserve(info%n*2)

        open(57, file=vFile, status='old', iostat=ierror, form='formatted')
        read(57, *, iostat=ierror)
        do i = 1, info%n
            if( present(flag) ) then ! city
                read(57, *, iostat=ierror) buff, info%ids(i), buff, info%lons(i), info%lats(i)
            else ! site
    ...
    subroutine read_raw_mdl(vFile, nhour, locs, data3d)
        ...
        ! Input Args
        character(*), intent(in)  :: vFile
        integer, intent(in) :: nhour ! 锁定到某个小时
        type(hash), intent(inout) :: locs
    ...
    subroutine read_raw_adj(vFile, data2d)
        ! 读取调优系数
        implicit none
        ! Input Args
        character(*), intent(in)   :: vFile
        ...
        real, dimension(size(data2d, 1)) :: data1d
        ...
        do i = 1, size(data2d, 2)
            read(50, *, iostat=ierror) buff, data1d
            data2d(:, i) = data1d
        end do
        close(50)
    ...
    subroutine write_data_csv(vFile, data, info, varNames)
        ! 写出调优系数表
        implicit none
        ! Input Args
        character(*), intent(in)   :: vFile
        ...
        write(99, 103) 'FLAG,       ID,   name,       lon,       lat, MM,  HH', (ADJUSTR(trim(varNames(i))), i=1, size(varNames))
        do i = 1, info%n
